...
function [Imout] = GSC_WLp_GST(InputImage, par, p )
...
for  i  =  1 : NN
    for  j  =  1 : MM
        ...
        lambda      =   2*sqrt(2)*par.sigma^2./(cc + par.e); %Eq.41
        
        tau =  lambda*step*Similar_patch/par.mu;

        Thre=1./(diag(GR_V)+0.1);%Eq.40
        
        GR_Z = GST_Shrink_ALG(GR_V,diag(tau.*Thre),p);%Eq.39
